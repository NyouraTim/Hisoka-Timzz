...
async function startpapah() {
    ...
    // Get Profile Picture Group
       ...
       papah.send5ButImg(pea[0].id, `「 *Group Settings Change* 」\n\nGroup telah ditutup oleh admin, Sekarang hanya admin yang dapat mengirim pesan !`, `Group Settings Change Message by Timzz Dev`, wm_papahdev, [])
       } else if(pea[0].announce == false) {
       papah.send5ButImg(pea[0].id, `「 *Group Settings Change* 」\n\nGroup telah dibuka oleh admin, Sekarang peserta dapat mengirim pesan !`, `Group Settings Change Message by Timzz Dev`, wm_papahdev, [])
       } else if (pea[0].restrict == true) {
       papah.send5ButImg(pea[0].id, `「 *Group Settings Change* 」\n\nInfo group telah dibatasi, Sekarang hanya admin yang dapat mengedit info group !`, `Group Settings Change Message by Timzz Dev`, wm_papahdev, [])
       } else if (pea[0].restrict == false) {
       papah.send5ButImg(pea[0].id, `「 *Group Settings Change* 」\n\nInfo group telah dibuka, Sekarang peserta dapat mengedit info group !`, `Group Settings Change Message by Timzz Dev`, wm_papahdev, [])
       } else {
       papah.send5ButImg(pea[0].id, `「 *Group Settings Change* 」\n\nGroup Subject telah diganti menjadi *${pea[0].subject}*`, `Group Settings Change Message by Timzz Dev`, wm_papahdev, [])
     }
    })
        ...
        try {
         ...
         const reSize = async(buffer, ukur1, ukur2) => {
             ...
             resolve(ab)
                ...
                let butwel = [{ buttonId: 'hehehe', buttonText: { displayText: 'Welcome👋' }, type: 1 }]
                let butleav = [{ buttonId: 'hehehe', buttonText: { displayText: 'Sayonara👋' }, type: 1 }]
                let butselamat = [{ buttonId: '', buttonText: { displayText: 'SELAMAT' }, type: 1 }]
                let butsebar = [{ buttonId: '', buttonText: { displayText: 'SABAR' }, type: 1 }]
                let nyoutube = ('© TimzzDev\nFollow ig :\nhttps://instagram.com/nyour_tim')
                let teks1 = `*Halo kak @${num.split('@')[0]}*\n*selamat datang di Group*\n*${metadata.subject}*\n*jangan lupa patuhi rules yang ada di group ini yaaaa><*\n_~Admin_`
